local utils = require("ollpilot-nvim.utils") -- BUG: add 'ollpilot-nvim' before publish
local M = {}

local MODEL_SIZES = {
	S = "qwen2.5-coder:1.5b",
	M = "qwen2.5-coder:7b",
	L = "qwen2.5-coder:14b",
}
local current_size = "S"
-- Change model sized from Ollpilot window
vim.keymap.set("n", "<leader>os", function()
	current_size = "S"
	utils.config.model = MODEL_SIZES[current_size]
	utils.update_window_title(M.main_win, utils.config.model)
...
local function query_ollama_stream(prompt, on_token, on_done, useContext)
	local request_body = vim.json.encode({
		model = utils.config.model,
		prompt = prompt,
		stream = true,
	...
	local cmd = {
		"curl",
		"-N", -- disables buffering, important for streaming
		"-s",
		utils.config.host .. "/api/generate",
		"-H",
		"Content-Type: application/json",
	...
	vim.system(cmd, {
		...
		stdout = function(_, data)
			...
			-- Keep any incomplete line in partial
...
function M.send_prompt()
	...
	-- NOTE: DEPRECATED - for <full-response-at-once> generation:
	...
	local response = {}
	query_ollama_stream(prompt, function(token)
		table.insert(response, token)
		vim.schedule(function()
			vim.api.nvim_buf_set_lines(M.output_buf, 0, -1, false, vim.split(table.concat(response, ""), "\n"))
		end)
	end, function()
		vim.schedule(function()
			vim.api.nvim_buf_set_option(M.output_buf, "modifiable", false)
